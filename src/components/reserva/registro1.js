...
const Registro = () => {
  ...
  const verificarDisponibilidad = async (values) => {
    ...
    try {
      const response = await axios.get(`https://barber-production-1d18.up.railway.app/api/reservas1/disponibilidad?fecha_hora=${fechaHora}`);
  ...
  const registrarReserva = async (values) => {
    ...
    try {
      const response = await axios.post('https://barber-production-1d18.up.railway.app/api/reservas1', datos);
