...
const Registro = () => {
  ...
  const verificarDisponibilidad = async (values) => {
    ...
    try {
      const response = await axios.get(`http://localhost/barber/public/api/reservas3/disponibilidad?fecha_hora=${fechaHora}`);
  ...
  const registrarReserva = async (values) => {
    ...
    try {
      const response = await axios.post('http://localhost/barber/public/api/reservas3', datos);
